public class Camera {
    ...
    public Camera(Vec3 lookfrom, Vec3 lookat, Vec3 vup, double vfou, double aspect_ratio){
        double theta = degrees_to_radians(vfou);
        double h = Math.tan(theta/2);
        double viewport_height = 2.0*h;
        double viewport_width = aspect_ratio * viewport_height;
        //double focal_length = 1.0;
        Vec3 w = Vec3.unit_vector(lookfrom.sub(lookat));
 ...
 */
    ...
    Ray get_ray(double s, double t){
        ...
        return new Ray(origin, lower_left_corner.add(horizontal.mul(s)).add(vertikal.mul(t)).sub(origin));
    }
    /*
